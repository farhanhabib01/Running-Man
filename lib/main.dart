...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const MaterialApp(
      title: 'Runner Chase Cyber Full Edition',
...
class _StartScreenState extends State<StartScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _bounceAnimation = Tween<double>(begin: 0, end: 16).animate(CurvedAnimation(parent: _bounceController, curve: Curves.easeInOut));

    _introController = AnimationController(vsync: this, duration: const Duration(milliseconds: 1400))..forward();

    _titleFade = CurvedAnimation(parent: _introController, curve: const Interval(0.0, 0.45, curve: Curves.easeOut));
    _titleSlide = Tween<double>(begin: -30, end: 0).animate(CurvedAnimation(parent: _introController, curve: const Interval(0.0, 0.45, curve: Curves.easeOutBack)));
    _logoFade = CurvedAnimation(parent: _introController, curve: const Interval(0.2, 0.75, curve: Curves.easeOut));
    _logoScale = Tween<double>(begin: 0.3, end: 1.0).animate(CurvedAnimation(parent: _introController, curve: const Interval(0.2, 0.85, curve: Curves.elasticOut)));
    _logoRotate = Tween<double>(begin: -0.5, end: 0.0).animate(CurvedAnimation(parent: _introController, curve: const Interval(0.2, 0.75, curve: Curves.easeOutCubic)));
    _bottomFade = CurvedAnimation(parent: _introController, curve: const Interval(0.55, 1.0, curve: Curves.easeOut));
    _bottomSlide = Tween<double>(begin: 26, end: 0).animate(CurvedAnimation(parent: _introController, curve: const Interval(0.55, 1.0, curve: Curves.easeOutCubic)));
    ...
    _sirenController = AnimationController(vsync: this, duration: const Duration(milliseconds: 1800))..repeat();
    ...
    Navigator.of(context).pushReplacement(
      PageRouteBuilder(
        ...
        transitionsBuilder: (context, animation, secondaryAnimation, child) {
          ...
          final scale = Tween<double>(begin: 1.1, end: 1.0).animate(CurvedAnimation(parent: animation, curve: Curves.easeOutCubic));
            ...
            SafeArea(
              child: Column(
                children: [
                  ...
                  const Spacer(),
                  AnimatedBuilder(
                    animation: Listenable.merge([_bounceAnimation, _introController]),
                    builder: (context, child) {
                      return Opacity(
                          ...
                          child: Transform.scale(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                gradient: RadialGradient(colors: [Colors.cyan.withValues(alpha: 0.6), Colors.transparent]),
                                boxShadow: const [BoxShadow(color: Colors.cyanAccent, blurRadius: 30, spreadRadius: 5)],
                              ),
                              child: Center(
                                child: ClipOval(
                                  child: Image.asset(
                                    ...
                                    errorBuilder: (_, __, ___) => const Icon(Icons.flash_on_rounded, size: 70, color: Colors.cyanAccent),
                                  ),
                                ),
                              ),
                            ),
                      ...
                      child: isLoaded
                          ? AnimatedBuilder(
                              ...
                              child: ElevatedButton.icon(
                                ...
                                label: const Text('START CYBER RUN', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900, letterSpacing: 1.5)),
                          ...
                          : Column(
                              children: [
                                LinearProgressIndicator(value: loadingProgress, color: Colors.cyanAccent, backgroundColor: Colors.white24, minHeight: 12),
                                const SizedBox(height: 12),
                                Text('Loading High-End Shaders... ${(loadingProgress * 100).toInt()}%', style: const TextStyle(color: Colors.white70, fontWeight: FontWeight.bold)),
                              ],
                            ),
                    ),
                  ),
...
class CyberGridPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double i = -size.width; i <= size.width * 2; i += 60) {
      canvas.drawLine(Offset(centerX + (i - centerX) * 0.1, horizonY), Offset(i, size.height), paint);
    }
    ...
    for (double y = horizonY; y < size.height; y += spacing) {
      ...
      if (currentY > horizonY) {
        double alphaFactor = ((currentY - horizonY) / (size.height - horizonY)).clamp(0.0, 1.0);
  ...
  @override
  bool shouldRepaint(covariant CyberGridPainter oldDelegate) => oldDelegate.phase != phase;
}
...
class FloatingPopup {
  ...
  final String text;
  final Color color;
  FloatingPopup({required this.x, required this.y, required this.text, this.color = Colors.cyanAccent, this.life = maxLife});
}
...
class _GameScreenState extends State<GameScreen> with TickerProviderStateMixin {
  ...
  double speed = 0.007;
  int score = 0, highestScore = 0, coins = 0, highestCoins = 0, keys = 0, level = 1, highestLevel = 1;
  ...
  void setupGame() {
    ...
    items.clear();
    popups.clear();
    score = 0; coins = 0; keys = 0; level = 1; speed = 0.007;
    playerLane = 1; playerLaneAnim = 1.0;
    isIntroPhase = true; isGameOver = false; isPaused = false; showRevivePrompt = false;
    hasShield = false; hasMagnet = false; magnetTimer = 0; comboMultiplier = 1;
    tickCounter = 0; nextSpawnTick = 30;
  ...
  void updateGame() {
    ...
    if (hasMagnet) {
      ...
      if (magnetTimer <= 0) hasMagnet = false;
        ...
        for (var item in items) {
          if (item.type == ItemType.coin && (item.y - playerY).abs() < 0.5) item.lane = playerLane;
        }
      }
    }
    ...
    if (isJumping) {
      jumpTick++;
      if (jumpTick >= jumpDuration) { isJumping = false; jumpTick = 0; }
    }

    if (policeLane == playerLane && policeY - playerY < 0.08 && !isJumping) {
      if (hasShield) { hasShield = false; policeY = 1.2; }
      else { triggerReviveOrGameOver(); return; }
    }
    ...
    for (var item in items) { item.y += speed; }

    for (var item in List<FallingItem>.from(items)) {
      if (item.lane == playerLane && item.y > playerY - 0.06 && item.y < playerY + 0.06) {
        if (item.type == ItemType.coin) {
          coins++; score += (10 * comboMultiplier);
          popups.add(FloatingPopup(x: item.lane * 1.0, y: item.y, text: '+${10 * comboMultiplier}'));
          items.remove(item); SoundManager.playCoin();
        } else if (item.type == ItemType.keyItem) {
          keys++; score += (50 * comboMultiplier);
          popups.add(FloatingPopup(x: item.lane * 1.0, y: item.y, text: 'KEY! +1', color: Colors.cyanAccent));
          items.remove(item); SoundManager.playCoin();
        } else if (item.type == ItemType.shield) {
          hasShield = true; items.remove(item);
          popups.add(FloatingPopup(x: item.lane * 1.0, y: item.y, text: 'SHIELD ACTIVE! ???', color: Colors.cyanAccent));
        } else if (item.type == ItemType.magnet) {
          hasMagnet = true; magnetTimer = 250; items.remove(item);
          popups.add(FloatingPopup(x: item.lane * 1.0, y: item.y, text: 'COIN MAGNET ??', color: Colors.amberAccent));
        } else if (isJumping) {
          item.dodged = true;
        } else {
          if (hasShield) { hasShield = false; items.remove(item); }
          else { triggerReviveOrGameOver(); return; }
        }
      }
    }
    ...
    for (var p in popups) { p.y -= 0.007; p.life--; }
  ...
  void triggerReviveOrGameOver() {
    ...
    if (keys > 0) {
      setState(() { showRevivePrompt = true; reviveTimerSeconds = 4; });
      reviveTimer?.cancel();
  ...
  Widget build(BuildContext context) {
    ...
    double jumpOffset = isJumping ? sin((jumpTick / jumpDuration) * pi) * jumpHeight : 0;

    return Scaffold(
      backgroundColor: Colors.black,
      body: GestureDetector(
        onTap: () { if (isIntroPhase) setState(() { isIntroPhase = false; SoundManager.playBackgroundMusic(); }); },
        onHorizontalDragEnd: (d) { if (!isIntroPhase && !showRevivePrompt && (d.primaryVelocity ?? 0) > 0 && playerLane < laneCount - 1) playerLane++; else if (!isIntroPhase && !showRevivePrompt && (d.primaryVelocity ?? 0) < 0 && playerLane > 0) playerLane--; },
        ...
        child: Stack(
          children: [
            ...
            Positioned(
              ...
              child: DecoratedBox(
                decoration: BoxDecoration(
                  gradient: LinearGradient(colors: [Colors.grey[900]!, Colors.black, Colors.grey[900]!]),
                  border: Border.symmetric(vertical: BorderSide(color: Colors.cyanAccent, width: 3)),
                ),
              ),
            ),
            ...items.map((i) => Positioned(
                  ...
                  child: i.type == ItemType.coin 
                      ...
                      : (i.type == ItemType.keyItem 
                          ? const Icon(Icons.vpn_key_rounded, color: Colors.cyanAccent, size: 36)
                          : (i.type == ItemType.shield 
                              ? const Icon(Icons.shield_rounded, color: Colors.blueAccent, size: 40) 
                              : (i.type == ItemType.magnet 
                                  ? const Icon(Icons.fitness_center_rounded, color: Colors.pinkAccent, size: 40)
                                  : (i.type == ItemType.car 
                                      ? safeImage(i.carImage ?? 'assets/game/car1.png', width: 70, height: 100)
                                      : const Icon(Icons.block, color: Colors.orange, size: 36))))),
                )),
            ...popups.map((p) => Positioned(
                  left: roadLeft + p.x * laneWidth, top: p.y * size.height,
                  child: Text(p.text, style: TextStyle(color: p.color, fontSize: 18, fontWeight: FontWeight.w900, shadows: const [Shadow(color: Colors.black, blurRadius: 6)])),
                )),
            Positioned(
              left: roadLeft + policeLaneAnim * laneWidth + laneWidth / 2 - 25,
              top: policeY * size.height,
              child: const Icon(Icons.local_police_rounded, color: Colors.redAccent, size: 50),
            ),
            Positioned(
              ...
              child: Stack(
                ...
                children: [
                  if (hasShield) Container(width: 80, height: 80, decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: Colors.cyanAccent, width: 3), boxShadow: const [BoxShadow(color: Colors.cyanAccent, blurRadius: 15)])),
            ...
            if (!isIntroPhase)
              Positioned(
                ...
                child: Row(
                  ...
                  children: [
                    Text('SCORE: $score', style: const TextStyle(color: Colors.cyanAccent, fontSize: 16, fontWeight: FontWeight.w900)),
                    Text('COINS: $coins ??', style: const TextStyle(color: Colors.amber, fontSize: 16, fontWeight: FontWeight.w900)),
                    Text('KEYS: $keys ??', style: const TextStyle(color: Colors.cyanAccent, fontSize: 16, fontWeight: FontWeight.w900)),
                    Text('LVL: $level ?', style: const TextStyle(color: Colors.pinkAccent, fontSize: 16, fontWeight: FontWeight.w900)),
                  ],
                ),
              ),
            if (isIntroPhase)
              Positioned.fill(
                child: Container(
                  ...
                  child: const Center(
                    child: Text('TAP TO START CYBER RUN', style: TextStyle(color: Colors.cyanAccent, fontSize: 26, fontWeight: FontWeight.w900, letterSpacing: 2)),
                  ),
                ),
              ),
            if (showRevivePrompt)
                ...
                child: Container(
                  ...
                  child: Center(
                    child: Column(
                      ...
                      children: [
                        const Text('CAUGHT BY POLICE!', style: TextStyle(color: Colors.redAccent, fontSize: 28, fontWeight: FontWeight.w900)),
                        const SizedBox(height: 10),
                        Text('Revive with Key? (00:0$reviveTimerSeconds)', style: const TextStyle(color: Colors.white, fontSize: 16)),
                        const SizedBox(height: 20),
                        ElevatedButton.icon(
                          onPressed: useKeyToRevive,
                          icon: const Icon(Icons.vpn_key_rounded),
                          label: const Text('USE KEY & RESUME'),
                ...
                child: Container(
                  ...
                  child: Center(
                    child: Column(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        const Text('GAME OVER', style: TextStyle(color: Colors.redAccent, fontSize: 32, fontWeight: FontWeight.w900)),
                        const SizedBox(height: 15),
                        Text('Final Score: $score | Coins: $coins', style: const TextStyle(color: Colors.white, fontSize: 18)),
                        const SizedBox(height: 25),
                        ElevatedButton(onPressed: setupGame, child: const Text('PLAY AGAIN')),
                      ],
                    ),
                  ),
                ),
  ...
  Widget safeImage(String path, {required double width, required double height}) {
    return Image.asset(path, width: width, height: height, fit: BoxFit.contain, errorBuilder: (_, __, ___) => const Icon(Icons.person, size: 50, color: Colors.white));
  }
}
